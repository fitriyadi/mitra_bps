	...
	<div class="row">
		<div class="col-md-12 grid-margin stretch-card">
			<div class="card">
				<div class="card-body">
						...
						<h6 class="card-title">Nilai Seleksi
						</h6>
		...
		<div class="row">
			<div class="col-md-12 grid-margin stretch-card">
				<div class="card">
					<div class="card-body">
						<form class="forms-sample" action="?hal=performa/proses" method="POST" enctype="multipart/form-data">
							<input type="hidden" name="idmitra" value="<?=$idmitra?>">
							<h6 class="card-title">Nilai Performa
							</h6>
							<div class="table-responsive">
								<table id="" class="table">
									<thead>
									...
									<tbody>
										...
										$no=0;
										$idmitra=$_GET['id'];
										$sql="SELECT * FROM tb_kriteria where jenis='Performa'";
										foreach (_dataGetAll($mysqli,$sql) as $key => $value) :
											extract($value);
											?>
											<tr>
												...
												<td><?=$max?></td>
												<td width="20%">
													<input type="number" class="form-control" max="<?=$max?>" name="nilai[<?=$idkriteria;?>]" value="<?=_dataCustom($mysqli,"select nilai from tb_mitra_performa where idmitra='$idmitra' and idkriteria='$idkriteria'")?>"
													required="">
												</td>
											</tr>
										<?php endforeach; ?>
							...
							<div class="col-sm-12">
								<button type="submit" class="btn btn-primary mr-2 float-right" name="tambah">Simpan</button>
								<a class="btn btn-light" href="?hal=performa/belum">Batal</a>
							</div>
						</div>
					</div>
				</div>
